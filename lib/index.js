...
export async function main(argv) {
	...
	for (let hook of ['teardown', 'pre', 'action', 'post']) {
		...
		for (let migration of migrations) {
			...
			const hookNotExplicitlyInSource = hook in module && !(hook in rawModule)
			...
			
			{
				...
				const [found] = ['pre', 'post'].includes(hook)
					? [{}]
					: await app.sql`
						select
						...
						-- do not want to trigger teardown because pre ran (it always runs)
						and hook not in ('pre', 'post')
						;
					`;
