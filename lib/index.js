...
export async function main(argv) {
	...
	await app.sql.unsafe`
		...
		alter table pgmg.migration_hook
			add column if not exists revision int default 0
		;
		
		update pgmg.migration_hook set revision = 0 where revision is null;

		alter table pgmg.migration_hook alter column revision set not null
		;
	`;
	...
	for (let hook of ['teardown', 'pre', 'action', 'post']) {
		debugLog("entered main sequence loop", { hook });

		if (!argv.dev && hook === 'teardown') {
			debugLog("skipping", { hook });
			continue;
		}
		
		for (let migration of migrations) {
			let module = await import(P.resolve(process.cwd(), migration));
			if (hook === 'teardown' && module.dev === false) {
				debugLog("skipping", { hook });
				continue;
			}
			...
			if (module.archived) {
				debugLog('module is archived, continuing')
				continue
			}
			...
			const hookNotExplicitlyInSource = hook in module && !(hook in module)
			...
			
			{
				...
				const hookFunction = module[hook];

				debugLog("searching for any migration", module.name, {
					hook
				});

				debugLog("search for migration hooks", module.name, {
					hook
				});
				...
				const [anyDevHookFoundForThisModule] = ['pre', 'post'].includes(hook)
					? [{}]
					: 	// either match on hook for new migrations
						...
						`;
				debugLog("search for dev hook", module.name, {
					anyDevHookFound: anyDevHookFoundForThisModule
					,found
					,hook
					,hooks_count,
				});
				...
				if (hook === 'pre' || hook === 'post') {
					shouldContinue = !!module[hook]
				} else if (hook === 'teardown') {
					shouldContinue = argv.dev && anyDevHookFoundForThisModule
				} else if (hook === 'action') {
					shouldContinue = argv.prod && !found
						|| argv.dev && (!found || found.dev && module.dev !== false) 
				}

				// can't run a hook if it is not exported
				if (!hookFunction) {
				...
				runMigration: if (shouldContinue) {
					...
					if (dry) {
						hookNotExplicitlyInSource || argv.debug || console.log(hook + "::" + migration, "(dry)");
						break runMigration;
					}
					try {
						hookNotExplicitlyInSource || argv.debug || console.log(hook + "::" + migration);
						debugLog("resetting role");
						await app.sql.unsafe(`reset role`);

						// we don't check for found.dev, because the hook may have run but failed
						// and isn't recorded in that case
						if (hook === 'action' && argv.dev) {
							await teardown_pgmg_objects(clusterSQL, {
								migration_user
								,service_user,
							});
						}
						...
						if (hook === 'action') {
							await create_pgmg_objects(clusterSQL, {
								migration_user
								,service_user,
							});
						}
						...
						// inefficient, but to ensure roles are isolated between migrations
						if (module.connection) {
							await app.resetConnection(module.connection);
						}
					} catch (e) {
