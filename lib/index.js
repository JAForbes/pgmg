...
export async function main(argv) {
	...
	async function teardown_pgmg_objects(sql, { migration_user, service_user }) {
		...
		for (let target of [migration_user, service_user]) {
			await dropRole(sql, target, { ownedBy: true, cascade: true })
		}
	}
	...
	async function create_pgmg_objects(sql, { migration_user, service_user }) {
		...
		for (let target of [migration_user, service_user]) {
			...
			if (target === migration_user) {
				await createRole(sql, target, { with: 'superuser nologin'})
			} else if (target === service_user) {
				await createRole(sql, target, { with: 'noinherit nologin nocreatedb nocreaterole nosuperuser noreplication nobypassrls'})
			}
		}
	}
	...
	await app.sql.unsafe`
		...
		create table if not exists pgmg.global_statement (
			name text not null references pgmg.migration(name) on delete cascade
			,hook name text not null references pgmg.migration(name) on delete cascade
			,statement text not null
			,primary key (name, hook, statement)
		);
	`;
			...
			const module = {
				...
				,async pre(...args) {
					...
					if (rawModule.pre) {
						...
						const sql = args[0]
						await sql`set role ${migration_user}`
